...
- `GET /tasks` — open `- [ ]` checkboxes in the daily notes folder (`TASKS_SCAN_FOLDER`, default `00 System/Daily Notes`) (for Hermes's nightly Obsidian → Todo import): skips empty boxes, templates/dot-folders, Tasks-plugin recurring (`🔁`) lines, and any text repeated in 5+ notes (habit/template lines); parses a `📅 YYYY-MM-DD` due date. Optional repeated `?extra=<vault-relative .md path>` scans additional files too (404-style `error` if one doesn't exist)
- `GET /health`
